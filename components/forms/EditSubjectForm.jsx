...
import { logout, updateSubject } from "../../lib/queries";
import { useRouter } from "expo-router";


export default function EditSubjectForm({setModalVisible, title, id, onRefresh}) {
  ...
  const updateSubjectMutatio = useMutation({
  ...
  const handleCreation = ()=>{
    if(subjectTitle.length > 2){
      updateSubjectMutatio.mutate({name: subjectTitle, id: id})
  ...
  return (
    ...
    <View style={styles.modal}>
      <View style={styles.formCont}>
        <TextInput style={styles.input} placeholder="Subject Title" placeholderTextColor={'white'} onChangeText={setSubjectTitle} value={subjectTitle}/>
      </View>
      ...
      <Pressable style={({pressed})=>[styles.button, {backgroundColor: pressed? '#AC040B':'#E50452'}]}
        ...
        <Pressable style={({pressed})=>[styles.button, {backgroundColor: pressed? '#1C6907':'#30B00C'}]} onPress={handleCreation}>
          <Text style={{color: 'white'}}>Update Subject</Text>
        </Pressable>
      </View>
